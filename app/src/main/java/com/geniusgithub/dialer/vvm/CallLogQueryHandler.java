...
import com.geniusgithub.dialer.calllog.CallLogQuery;
import com.geniusgithub.dialer.util.AppCompatConstants;
import com.geniusgithub.dialer.util.TelecomUtil;
...
*/
    ...
    public void fetchVoicemailStatus() {
        if (TelecomUtil.hasReadWriteVoicemailPermissions(mContext)) {
            Log.i(TAG, "fetchVoicemailStatus");
            startQuery(QUERY_VOICEMAIL_STATUS_TOKEN, null, Status.CONTENT_URI,
                    VoicemailStatusHelperImpl.PROJECTION, null, null, null);
        }else{
            Log.i(TAG, "fetchVoicemailStatus no permission");
        }
    }
    ...
    private void fetchCalls(int token, int callType, boolean newOnly, long newerThan) {
        ...
        Uri uri = TelecomUtil.getCallLogUri(mContext).buildUpon()
                ...
                .build();
        Log.i(TAG, "startQuery calllog uri = " + uri.toString());
   ...
   /* public void markNewCallsAsOld() {
    ...
    protected synchronized void onNotNullableQueryComplete(int token, Object cookie,
            Cursor cursor) {
        Log.i(TAG, "onNotNullableQueryComplete token = " + token  + ", cursor = " + cursor);
        if (cursor == null) {
            return;
        }
        try {
